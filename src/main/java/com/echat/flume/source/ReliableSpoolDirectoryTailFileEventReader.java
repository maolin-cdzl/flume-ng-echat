package com.echat.flume.source;
...
import com.echat.flume.source.SpoolDirectoryTailFileSourceConfigurationConstants.ConsumeOrder;
import static com.echat.flume.source.SpoolDirectoryTailFileSourceConfigurationConstants.*;
...
public class ReliableSpoolDirectoryTailFileEventReader implements ReliableEventReader{
	...
	public List<Event> readEvents(int numEvents) throws IOException {
	    ...
	    List<Event> events = currentFile.get().getDeserializer().readEvents(numEvents);
	    ...
	    if (events.isEmpty()) {
	         ...
	         */
	       if(!isTargetFile(currentFile) && (isExistNextFile()) ){	
			   //	Only CurrentFile is no longer the target, at the meanwhile, next file exists.
			   //	Then deal with the history file(ever target file)
